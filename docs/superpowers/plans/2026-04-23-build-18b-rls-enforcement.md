...
- **ConfigProvider auth-race on cold sessions (cosmetic, surfaced 2026-04-25 Step 10 smoke):** `src/lib/config-context.tsx` fires `supabase.from("damage_types")` and `supabase.from("job_statuses")` immediately on mount. On a cold incognito session, the request can fire before the auth cookie is available to the browser client, going out as `anon`. Pre-build57 the `Allow all on damage_types` / `Allow all on job_statuses` legacy policies granted access to `public`/`anon`, so this race was masked. Post-build57 the request returns 0 rows under `tenant_isolation_*` (which is `{authenticated}` only), and `damageTypes`/`statuses` arrays stay empty for that page-load. Visual symptom: job-card colored top strip falls back to gray (`#666666`) on `/jobs`; status badges stay correct because `getStatusColor()` has a hardcoded `DEFAULT_STATUS_COLORS` fallback while `accentColor = dtConfig?.text_color || "#666666"` does not. Normal warm sessions are unaffected. **Fix candidates:** (a) gate ConfigProvider's fetch on `supabase.auth.getSession()` resolving with a session, or (b) add a fallback hardcoded color table for damage types analogous to `DEFAULT_STATUS_COLORS`, or (c) add a narrow `anon` SELECT policy on the global `organization_id IS NULL` reference rows in `damage_types` and `job_statuses` (these tables hold non-sensitive lookup data: water/fire/mold + status names). Option (c) is the cleanest if we accept that those reference rows are public; otherwise (a) is the principled fix. No tenant-isolation impact either way.
