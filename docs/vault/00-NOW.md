...
[[build-67a]] **Estimates & Invoices Foundation — server lib + 3 API routes shipped, plan part-2 complete (~30% of full 67a).** Two sessions today on TheLaunchPad: morning shipped Tasks 1–4 (spec, plan part-1, schema migration to prod, TS types, format helper) and afternoon ran SDD on Tasks 5–9 + wrote plan part-2. Tasks 5–9 (commits `d4d468e` → `4e53cc3` on main): `src/lib/estimates.ts` (recalc + numbering + loaded fetch + depth assert), `src/lib/item-library.ts` (CRUD), `POST/GET /api/estimates`, `GET/PUT/DELETE /api/estimates/[id]`, `POST/PUT /api/estimates/[id]/sections`. C1 bug caught in Task 9 code review (PostgREST `.is()` rejects UUIDs) fixed at `4e53cc3`. Plan part-2 (`debb9ef`, 1797 lines) details Tasks 10–14 with full code (5 remaining API routes — line-items × 2, sections[id], item-library × 2; hardening lessons baked in) and Tasks 15–30 with milestone-detail (sidebar nav, Item Library page, estimate creation entry, ~10 builder components, dnd-kit + auto-save wiring, read-only view, job-page integration, final audit). **21 tasks remaining**. Two follow-up chips spawned: PostgREST `.or()` ilike injection sweep (3 callers) and 67a-routes hardening sweep (Tasks 7–9 backport — JSON parse guards, ID type checks, length caps, `voided_by` audit column, TOCTOU on optimistic concurrency, void idempotency, atomic reorder RPC). See [[2026-04-30-build-67a-2]] for full Session 2 handoff; [[2026-04-30-build-67a]] for Session 1.
...
- `main` at `debb9ef` — `plan: build 67a part-2 (Tasks 10–30)`. Earlier same-day commits on main: 67a Tasks 1–9 (`f626ec8` → `4e53cc3`), Build 14j (`a651057` and earlier), two intra-day vault handoffs (`aae65d0`, `02f308c`).
...
- [[build-67a]] continuation: 21 of 30 tasks remaining. **Resume point: SDD Tasks 10–14 in a fresh session** — plan part-2 has full code blocks for those 5 API routes (with hardening lessons from Tasks 7–9 baked in). Then Tasks 15–18 (sidebar nav + Item Library page + ItemForm + ItemTable). Then Tasks 19–30 in 2–3 more sessions for the estimate builder UI, read-only view, job-page integration, final audit. Each Task 15–30 has milestone-detail in plan part-2; the SDD implementer subagent should read the spec doc § referenced + named reference files (form-builder dnd, Tiptap, etc.) per task.
- Two follow-up chips filed for [[build-67a]] route hardening (independent of forward progress): PostgREST `.or()` ilike injection sweep across 3 callers, and Tasks 7–9 hardening backport (embedded checklist in plan part-2). Run any time.
